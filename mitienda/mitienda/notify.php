...
curl_setopt($ch, CURLOPT_mysqli_connECTTIMEOUT, 30);
curl_setopt($ch, CURLOPT_HTTPHEADER, array('mysqli_connection: Close'));
$res = curl_exec($ch);
if (curl_errno($ch) != 0) {
    error_log(date('[Y-m-d H:i e] ') . "Can't mysqli_connect to PayPal to validate IPN message: " . curl_error($ch) . PHP_EOL, 3, 'app.log');
    curl_close($ch);
    exit;
} else {
